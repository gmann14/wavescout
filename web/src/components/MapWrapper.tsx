...
export default function MapWrapper() {
  ...
  const [gallerySpotCount, setGallerySpotCount] = useState(0);
  ...
  useEffect(() => {
    ...
    import("mapbox-gl").then((mapboxgl) => {
      ...
      if (!token) {
        setLoadError("Set NEXT_PUBLIC_MAPBOX_TOKEN to render the map.");
        return;
      }
      ...
      map.on("load", async () => {
        try {
          ...
          setSpotCount(spots.features.length);
          setGallerySpotCount(galleryData.spots.length);
          setLoadError(null);
          ...
          map.on("click", "spots-dot", (e: any) => {
            ...
            setSelectedSpot(normalizeSpotProperties(e.features[0].properties as Record<string, unknown>));
          });
          ...
          console.error(error);
          setLoadError("Failed to load the public spot dataset.");
        }
      });
    });
    ...
    <>
      ...
      <div className="absolute bottom-4 left-4 bg-[#0f1629]/90 backdrop-blur border border-[#1e2d4d] rounded-lg p-3 text-xs space-y-1.5 z-10 hidden sm:block">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-slate-300">Named spots ({spotCount || "..."})</span>
        </div>
      </div>

      {/* Stats bar */}
      <div className="absolute top-4 left-4 bg-[#0f1629]/90 backdrop-blur border border-[#1e2d4d] rounded-lg px-3 py-2 text-xs z-10">
        <span className="text-white font-bold">{spotCount}</span>
        <span className="text-slate-500"> named spots</span>
        <span className="text-slate-600 mx-1.5">|</span>
        <span className="text-teal-400 font-bold">{gallerySpotCount}</span>
        <span className="text-slate-500"> gallery spot sets</span>
      </div>
