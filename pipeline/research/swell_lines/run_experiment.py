...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the frozen 8-scene swell-line experiment.")
    ...
    parser.add_argument("--band", default=DEFAULT_SIGNAL_BAND, help="Signal band to read, e.g. B04 or B08.")
    parser.add_argument("--output", type=Path, default=RESULTS_PATH, help="Results JSON output path.")
    parser.add_argument("--noise-floor-quantile", type=float, default=0.5, help="Noise floor quantile for detector.")
    parser.add_argument("--min-peak-snr", type=float, default=6.0, help="Minimum spectral peak SNR for organized scenes.")
    parser.add_argument(
        "--min-peak-fraction",
        type=float,
        default=0.05,
        help="Minimum local peak fraction for organized scenes.",
    )
...
def _organized_scene_pass(scene: dict, result) -> tuple[bool, dict]:
    wavelength_ok = result.wavelength_m is not None and 80.0 <= result.wavelength_m <= 250.0
    azimuth_delta = angular_diff_mod180(result.azimuth_deg, scene.get("swell_direction_deg"))
...
def main() -> None:
    ...
    payload = run_experiment(
        ...
        detector_kwargs={
            "noise_floor_quantile": args.noise_floor_quantile,
            "min_peak_snr": args.min_peak_snr,
            "min_peak_fraction": args.min_peak_fraction,
        },
    )
    print(json.dumps({"status": payload["status"], "scenes_correct": payload["scenes_correct"], "scene_count": payload["scene_count"]}, indent=2))
